...
var (
	//nolint:gochecknoglobals // Constant lookup table for process type names
	...
	//nolint:gochecknoglobals // Constant lookup table for process initialization names
	...
	//nolint:gochecknoglobals // Constant lookup table for process completion names
...
func (c *Client) Restart(ctx context.Context,
	...
	// Add a small delay to ensure containers are fully stopped before starting
	select {
	case <-ctx.Done():
		return ctx.Err()
