...
func TestRestart(t *testing.T) {
	...
	// Verify Redis is up before restart
	assert.Assert(t, redisIsUp(t, redisPort), "Redis should be up before restart")

	// Stop Redis first
	assert.NilError(t, dockerClient.Stop(ctx, service.Redis), "failed to stop container")

	// Verify Redis is down
	assert.Assert(t, redisIsDown(t, redisPort), "Redis should be down after stop")

	// Start Redis again
	assert.NilError(t, dockerClient.Start(ctx, service.Redis), "failed to start container after stop")

	// Test if the container is running
	assert.Assert(t, redisIsUp(t, redisPort), "Redis should be up after restart")
}
...
func cleanUp(t *testing.T, dockerClient *Client) {
	t.Cleanup(func() {
		// Use a separate context for cleanup operations to avoid context cancellation
		// The test context gets canceled when the test completes, but cleanup needs to run after
		ctx := context.Background() //nolint:usetesting // Need background context for cleanup after test completion
		assert.NilError(t, dockerClient.Purge(ctx, service.Nakama,
			service.Cardinal, service.Redis, service.NakamaDB, service.Jaeger, service.Prometheus),
